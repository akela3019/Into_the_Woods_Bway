...
theme_custom <- theme(
  ...
  axis.text = element_text(color = "#757574", size = 9.5),
...
geom_production_label <- function() {
  list(geom_label(data = itw_data %>% group_by(Production) %>% slice(1),
...
theatres <- data.frame(
  ...
  theatre = c("AL HIRSCHFELD", "BROADHURST", "ST. JAMES"),
...
label_df <- rbind(
  ...
  cbind(subset(itw_data, WeekEnding == as.Date("1988-04-17")),
        shape = "Cast Change (Witch: Joslyn\u2192Rashad)", label = "\U1F504")) %>%
  mutate(shape = factor(shape, c("Xmas/New Year", "Labor Day", "PBS Filming", 
                                 "Cast Change (Witch: Joslyn\u2192Rashad)"))) %>%
  mutate(x_pos = case_when(shape == "Xmas/New Year" ~ Week + 0.3,
                           shape == "Labor Day" ~ Week + 0.2,
                           shape == "PBS Filming" ~ Week + 0.3,
                           TRUE ~ Week))
...
p_attendance <- itw_data %>%
  ...
  mutate(max_attenance = (Perf + Previews) * n_seats) %>% 
  ggplot(aes(x = WeekEnding, color = Production, fill = Production)) +
  geom_col(aes(y = Attendance), color = "#e8ece5", linewidth = 0.05) +
  geom_line(aes(y = max_attenance), color = "#9b3950", linewidth = 0.4, linetype = "11") +
  geom_blank(data = dummy) +
  geom_production_label() +
  ggforce::facet_col(Production ~ ., scales = "free", space = "free") +
  theme_custom +
  ...
  scale_y_continuous(breaks = seq(0, 24, 6) * 1E3, expand = expansion(mult = c(0.01, 0.01)),
...
p_capacity <- 
  ...
  geom_blank(data = dummy) +
  geom_point(data = label_df[1:(nrow(label_df)-2), ], 
             aes(x = WeekEnding - 1, shape = shape), size = 2.5,
             color = "black") +
  ...
  scale_fill_stepsn(colors = rev(brewer.pal(11, "RdYlBu")), 
                    ...
                    labels = function(x) {paste0(x, "%")}) +
  scale_shape_manual(values = c("\U1F384", "\U1F6E0", "\U1F3A5", "\U1F504"), name = NULL) +
  guides(fill = guide_colorbar(barwidth = 15, barheight = 0.3, ticks = FALSE, order = 1),
         shape = guide_legend(order = 2, keywidth = 1, keyheight = 0.3, ncol = 1,
                              label.theme = element_text(margin = margin(t = 1.5, b = 1.5), color = swatch[4], size = 10,
                                                         family = "Franklin Gothic Medium Cond", vjust = 0.5)))+
...
p_avg_gross <- itw_data %>%
  ...
  geom_text(data = data.frame(x = 60, y = 4.5E4, label = "The Week After\nLabor Day"), 
            aes(x = x, y = y, label = label), color = "#9b3950", 
            family = "Franklin Gothic Medium Cond", size = 3,
            inherit.aes = FALSE, lineheight = 1, hjust = 0) +
  geom_curve_arrow() +
  # geom_curve(data = labor_day[1, ], linetype = "11",
  #            aes(xend = Week + 1.5, yend = Avg_Gross_Adjusted - 2E3, x = 60 - 1, y = 5E4),
  #            color = "#9b3950", curvature = -0.15, linewidth = 0.4,
  #            arrow = arrow(angle = 20, length = unit(0.06, "inches"), type = "closed")) +
  # geom_text(data = labor_day, aes(label = "\U1F6E0"), show.legend = FALSE, size =3) +
  scale_x_week() +
  ...
  theme(panel.grid.major.y = element_line(linetype = "12", linewidth = 0.4),
        ...
        legend.text = element_markdown(lineheight = 1, family = "Dubai", 
...
p_perf <- itw_data %>%
  ...
  geom_text(aes(x = text_pos, 
                label = abs(n), color = text_col), 
            fontface = "bold", position = position_stack(vjust = 0.5),
            family = "Dubai", hjust = 0.5, vjust = 0.4, size = 3.5) +
...
left_col <- plot_grid(
  ...
  ncol = 1, align = "v", rel_heights = c(3.5, 3.1, 3))

right_col_top <- plot_grid(
  p_avg_gross, p_price_adjusted, 
...
right_col <- plot_grid(
  ...
  ncol = 1, align = "v", rel_heights = c(7, 2.2))
...
g <- plot_grid(left_col, right_col, nrow = 1, rel_widths = c(5, 4)) %>%
  plot_grid(title, ., caption, ncol = 1, rel_heights = c(0.035, 1, 0.025)) +
  theme(plot.background = element_rect(color = NA, fill = "#e8ece5"),
        plot.margin = margin(r = 5, t = 5))

ggsave("itw_data.png", g, width = 10, height = 7.2, dpi = 600)
